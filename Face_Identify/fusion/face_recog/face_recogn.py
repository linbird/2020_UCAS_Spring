...
def face_encodings(face_image, known_face_location=None, num_jitters=1, model="small"):
    if(type(face_image) == "tulpe"):
        known_face_location = face_image[1]
...
#    return [np.array(face_encoder.compute_face_descriptor(face_image, raw_landmark_set, num_jitters)) for raw_landmark_set in raw_landmarks]
##    res = [np.array(face_encoder.compute_face_descriptor(face_image, raw_landmark_set, num_jitters)) for raw_landmark_set in raw_landmarks]
##    print("res")
##    return res
...
def batch_face_encodings(face_images, known_face_locations=None, num_jitters=1):
    ...
    if known_face_locations is None:
        raw_landmarks = [_raw_face_landmarks(
            face_image, None, model="small") for face_image in face_images]
    else:
        raw_landmarks = [
            _raw_face_landmarks(
                face_image, this_known_face_locations, model="small")
